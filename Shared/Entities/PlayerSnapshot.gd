...
func decode(data: PoolByteArray):
	self.tick = bytes2var(NetEncoding.HEADERS.int + data.subarray(0, 3))
	self.player_position = bytes2var(NetEncoding.HEADERS.vec3 + data.subarray(4, 15))
	self.player_angle = bytes2var(NetEncoding.HEADERS.float + data.subarray(16, 19))
	self.head_angle = bytes2var(NetEncoding.HEADERS.float + data.subarray(20, 23))
	return self
